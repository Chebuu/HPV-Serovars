...
Add2DB(HPV.REF.PaVE.vars %>% mutate(identifier = HPVREHum), dbConn)
...
params <- expand.grid(
  ...
  resolution = seq(75, 300, length.out = 3), 
...
gridsearch <- apply(
  params %>% 
    mutate(
      dbConn = rep(dbConn, nrow(params)), 
      type = rep('melt', nrow(params))
    ), 
  1, 
  function(arg) 
    list(arg = arg, out = do.call(DesignSignatures, as.list(p)))
) %>% bind_rows
