...
DECIPHER::Add2DB(HPV.REF.PaVE.vars, dbConn, verbose = F)
...
data(HPV.REF.PaVE.seqs)
data(HPV.REF.PaVE.vars)
Seqs2DB(HPV.REF.PaVE.seqs, 'XStringSet', dbConn, '')
Add2DB(data.frame(identifier = as.character(HPV.REF.PaVE.vars)), dbConn)
...
gridsearch <- apply(params, 1, function(...) 
    list(arg = as.list(...), out = do.call(DesignSignatures, list(dbFile = dbConn, tblName = 'Seqs', identifier = '', ...)))
) %>% bind_rows
